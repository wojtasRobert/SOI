...
/*-----------------------------------------*/
/* dodane funkcje systemowe na koncu pliku */
/*-----------------------------------------*/
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *max_hole, *prev_to_max;
  ...
  if(!worst_fit){
    ...
    do
    {
      hp = max_hole = hole_head;	/* start at the beginning of the list */
      prev_to_max = prev_ptr = NIL_HOLE;
      while(hp != NIL_HOLE && hp->h_base < swap_base)
      {
        if(hp->h_len > max_hole->h_len)	/* compare size of the hole */
        {
          prev_to_max = prev_ptr; /* remember previous to max */
          max_hole = hp;
        }
        prev_ptr = hp;	/* remember previous hole */
        hp = hp->h_next;
      }
      /* found the biggest hole and it's big enough */
      if(max_hole != NIL_HOLE && max_hole->h_len >= clicks)
      {
        old_base = max_hole->h_base;
        max_hole->h_base += clicks; /* bite a piece off */
        max_hole->h_len -= clicks; /* ditto */
        /* Delete the hole if used up completely. */
        if (max_hole->h_len == 0) del_slot(prev_to_max, max_hole);
        return (old_base);
      }
    } while(swap_out()); /* try to swap some other process out */
  }
...
int do_hole_map()
{
  int nbytes = mm_in.m1_i1;		/* buffer size */
  char *usr_buffer = mm_in.m1_p1;	/* buffer address */
  int usr_pid = mm_in.m_source;
  struct hole *hp = hole_head;		/* start at the beginning */
  int i = 0;
  phys_clicks buffer[(NR_HOLES)*2 + 1]; /* worst case scenario size - all holes are free */

  /* how many pairs will fit in usr_buffer with leading 0 */
  int nr_pairs = (nbytes/(sizeof(phys_clicks)) - 1)/2;
  if (nr_pairs < 0) nr_pairs = 0;
  
  if(nbytes < sizeof(phys_clicks)) return -1; /* cant even put in a 0 */

  while(hp != NIL_HOLE && hp->h_base < swap_base && nr_pairs > 0)
  {
    buffer[i] = hp->h_len;
  ...
  buffer[i] = 0;
  sys_copy(0,D,(phys_bytes) buffer, usr_pid, D, (phys_bytes) usr_buffer, (i+1)*sizeof(phys_clicks));
  return (i/2);
}
